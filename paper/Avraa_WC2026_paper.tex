...
squad and injury news monitored through 9 June, when the confirmed withdrawals of Jurri\"en Timber (Netherlands) and several Brazil attackers were already in the pre-kickoff ratings and the remaining late losses (e.g.\ Germany's Lennart Karl) fall on non-decisive squad members, so the entry is confirmed unchanged;
...
\begin{table}[htpb!]
  ...
  \begin{threeparttable}
    ...
    \end{tabular*}}
    \begin{tablenotes}\notesize
      \item Note: Collected June 5--7, 2026.
    \end{tablenotes}
  \end{threeparttable}
  \end{small}
\end{table}
...
\begin{table}[htpb!]
  ...
  \begin{threeparttable}
    ...
    \end{tabular*}}
    \begin{tablenotes}\notesize
      \item Note: $N$ = 200{,}000; standard errors $\le$ 0.11 pp.
    \end{tablenotes}
  \end{threeparttable}
  \end{small}
\end{table}
...
\begin{figure}[htpb!]
  ...
  \begin{threeparttable}
    ...
    \begin{tablenotes}\notesize
      \item Note: Champion probability across 200{,}000 simulated tournaments. Spain (blue) is the modal champion yet fails to win in 72.6\% of simulations; Portugal (orange) is discussed in Section~\ref{sec:portugal}. The 36 teams outside the top twelve jointly retain a 9.7\% champion share.
    \end{tablenotes}
  \end{threeparttable}
\end{figure}
...
\begin{figure}[htpb!]
  ...
  \begin{threeparttable}
    ...
    \begin{tablenotes}\notesize
      \item Note: Fitted Poisson scoreline distribution for the predicted final, Spain vs.\ Argentina. The orange box marks the submitted prediction (1--0), which carries only 13.2\% probability; no single scoreline is much more likely.
    \end{tablenotes}
  \end{threeparttable}
\end{figure}
...
\begin{figure}[htpb!]
  ...
  \begin{threeparttable}
    ...
    \begin{tablenotes}\notesize
      \item Note: Model and market champion odds at the baseline, before any match is played. The model (blue) is more concentrated on Spain and Argentina, while the market (orange) spreads more probability to England, Brazil, and Germany. The disagreement comes entirely from the knockout stage, because the model's group-stage probabilities are themselves recovered from market odds.
    \end{tablenotes}
  \end{threeparttable}
\end{figure}
...
\begin{figure}[htpb!]
  ...
  \begin{threeparttable}
    ...
    \begin{tablenotes}\notesize
      \item Note: Forecast evolution replayed on the 2018 and 2022 World Cups. Top: the champion probability of the eventual winner (bold) and three reference teams, conditioned on actual results, rising from the pre-tournament prior to one. Bottom: the information each result carried (orange for group matches, blue for knockouts). In both tournaments the largest spike is the elimination of Brazil, the pre-tournament favorite, and the group stage carries little information.
    \end{tablenotes}
  \end{threeparttable}
\end{figure}
...
\end{minipage}}
    \begin{tablenotes}\notesize
      \item Note: Qualification probabilities by team (percent), 100{,}000 simulations. Columns report the probability of winning the group, finishing second, advancing as a best third-placed team ($^{*}$), and qualifying in total.
    \end{tablenotes}
  \end{threeparttable}
\end{table}
...
\begin{table}[htpb!]
  ...
  \begin{threeparttable}
    ...
    \end{tabular*}}
    \begin{tablenotes}\notesize
      \item Note: The decisive fixture is the match that, in the simulations, most often determines the second qualification place; the primary risk is the most probable way the submitted group order is wrong.
    \end{tablenotes}
  \end{threeparttable}
  \end{small}
\end{table}
...
Curaçao                &  6.8\% &  0.6\% &  0.1\% &  0.0\% &  0.0\% &  0.0\% \\
    ...
    \end{tabular*}}
    \begin{tablenotes}\notesize
      \item Note: Cumulative probabilities, recalibrated model, $N$ = 200{,}000.
    \end{tablenotes}
  \end{threeparttable}
  \end{small}
\end{table}
...
\begin{table}[htpb!]
  ...
  \begin{small}
  \begin{threeparttable}
    {\scriptsize\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} lcccccccc}
      \toprule
    ...
    \end{tabular*}}
    \begin{tablenotes}\notesize
      \item Note: Champion probabilities (\%) as model layers are added.
    \end{tablenotes}
  \end{threeparttable}
  \end{small}
\end{table}
...
\begin{table}[htpb!]
  ...
  \begin{small}
  \begin{threeparttable}
    {\scriptsize\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} l *{9}{c}}
      \toprule
    ...
    \end{tabular*}}
    \begin{tablenotes}\notesize
      \item Note: 2018 group-stage replay ($N=3{,}000$). A genuine out-of-sample test, because France won its group unconvincingly; learning still helps, with a flat optimum near $k=60$ (bold).
    \end{tablenotes}
  \end{threeparttable}
  \end{small}
\end{table}
...
\begin{table}[htpb!]
  ...
  \begin{threeparttable}
    ...
    \end{tabular*}}
    \begin{tablenotes}\notesize
      \item Note: Top: adversarial scenarios probing the learning engine and data layer (all passed). Bottom: robustness of the headline finding, the learning track's lift of the eventual finalists' champion mass over the frozen track, under seed, rating, and proxy perturbations.
    \end{tablenotes}
  \end{threeparttable}
  \end{small}
\end{table}
