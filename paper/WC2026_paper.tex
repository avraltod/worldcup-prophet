...
\begin{figure}[!t]
  ...
  \begin{threeparttable}
    ...
    \begin{tablenotes}\notesize
      \item \textit{Notes}: The live study on real results, through 3 of 104 matches. Upper panel: each contender's champion probability after every recorded result, starting from the pre-tournament baseline. Lower panel: the information content of each result in bits (Equation~\ref{eq:kl}). The figure is regenerated after every match; the dress-rehearsal version of this instrument, validated on a simulated tournament, is retained in the locked pre-kickoff edition.
    \end{tablenotes}
  \end{threeparttable}
\end{figure}
