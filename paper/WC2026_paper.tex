...
\begin{figure}[!t]
  ...
  \begin{threeparttable}
    ...
    \begin{tablenotes}\notesize
      \item \textit{Notes}: Fitted Poisson scoreline distribution for the predicted final, Spain vs.\ Argentina. The orange box marks the submitted prediction (2--1), which carries only 8.2\% probability; even the single most likely scoreline, 1--0, reaches just 13.2\%, so no scoreline is close to certain.
    \end{tablenotes}
  \end{threeparttable}
\end{figure}
