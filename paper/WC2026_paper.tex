...
\begin{table}[!t]
  ...
  \begin{threeparttable}
    {\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} lcccc}
      ...
      \midrule
      Spain          & 26.9\% & 26.9\% & 38.8\% & 51.7\% \\
      Argentina      & 18.1\% & 17.9\% & 30.1\% & 43.1\% \\
      France         & 14.3\% & 14.3\% & 24.6\% & 43.3\% \\
      Portugal       & 9.4\% & 9.3\% & 18.1\% & 29.2\% \\
      England        & 7.0\% & 7.1\% & 14.6\% & 28.3\% \\
      Brazil         & 3.5\% & 3.6\% & 8.6\% & 20.3\% \\
      Netherlands    & 2.4\% & 2.3\% & 6.1\% & 15.9\% \\
      Germany        & 2.2\% & 2.2\% & 6.3\% & 16.6\% \\
      \bottomrule
    \end{tabular*}}
    \begin{tablenotes}\notesize
      \item \textit{Notes}: ``now'' columns conditioned on the 2 results revealed to date ($N$ = 50{,}000, seed 2026); the lock column is the $N$ = 200{,}000 pre-kickoff baseline (\texttt{data/frozen\_stage\_probs.json}) and never changes.
    \end{tablenotes}
  \end{threeparttable}
  \end{footnotesize}
\end{table}
...
\begin{figure}[!t]
  ...
  \begin{threeparttable}
    ...
    \begin{tablenotes}\notesize
      \item \textit{Notes}: The live study on real results, through 2 of 104 matches. Upper panel: each contender's champion probability after every recorded result, starting from the pre-tournament baseline. Lower panel: the information content of each result in bits (Equation~\ref{eq:kl}). The figure is regenerated after every match; the dress-rehearsal version of this instrument, validated on a simulated tournament, is retained in the locked pre-kickoff edition.
    \end{tablenotes}
  \end{threeparttable}
\end{figure}
