...
\begin{tikzpicture}[
  font=\footnotesize,
  >={Stealth[round]},
  io/.style={rectangle, rounded corners=2pt, draw=blue!55!black, fill=blue!6, align=left, text width=66mm, inner sep=4pt},
  pbox/.style={rectangle, draw=black!70, fill=white, align=center, text width=66mm, inner sep=4pt, minimum height=7mm},
  learn/.style={rectangle, draw=orange!80!black, fill=orange!9, align=center, text width=66mm, inner sep=4pt},
  sim/.style={rectangle, draw=black!70, fill=black!5, align=center, text width=33mm, inner sep=3pt, minimum height=10mm},
  dec/.style={diamond, draw=black!75, fill=black!4, aspect=2.4, align=center, inner sep=1pt},
  term/.style={rectangle, rounded corners=7pt, draw=black!85, fill=black!7, align=center, text width=70mm, inner sep=4pt},
  a/.style={->, thick, black!75},
  node distance=4mm,
]
\node[io] (inp) {\textbf{INPUTS — pre-tournament feeds}\\[1pt]
  $\bullet$ baseline Elo ratings $R_0$ (frozen reference)\\
  $\bullet$ market / bookmaker odds\\
  $\bullet$ structure $S$: groups, fixtures, bracket};
\node[pbox, below=of inp] (init) {\textbf{1. Initialise}\\
  frozen $\leftarrow R_0$;\quad $L \leftarrow$ LearningTrack($R_0$, drift $=0$);\quad results $\leftarrow \emptyset$};
\node[pbox, below=of init] (t0) {\textbf{2.} $t{=}0$ baseline snapshot $\rightarrow$ simulate both tracks $\rightarrow$ champion distribution};
\node[draw=black!55, dashed, fill=black!2, rounded corners, below=5mm of t0, text width=72mm, align=center, inner sep=2pt] (loophead) {\textbf{LOOP} \; $\Rightarrow$ \; for each game $g$ as the tournament reveals it};
\node[io, below=4mm of loophead] (bef) {\textbf{3. BEFORE kickoff — gather \& feed}\\[1pt]
  $\bullet$ confirmed starting lineups\\
  $\bullet$ late injury / suspension news\\
  $\bullet$ strengths learned so far ($L$)};
\node[pbox, below=of bef] (befsim) {\textbf{4.} simulate both tracks with current state\\ $\rightarrow$ record \emph{before} snapshot ($+$ KL)};
\node[font=\scriptsize\itshape, below=2.5mm of befsim] (play) {\quad$\cdots$ game $g$ is played $\cdots$\quad};
\node[io, below=2.5mm of play] (aft) {\textbf{5. AFTER full-time — gather \& feed}\\[1pt]
  $\bullet$ final result / scoreline (winner)\\
  $\bullet$ shots, shots on target, possession\\
  $\bullet$ corners, cards $\;\rightarrow\;$ $\lambda_{\text{obs}}$ (xG proxy, or free real xG)};
\node[pbox, below=of aft] (cond) {\textbf{6.} condition: results $\leftarrow$ results $\cup\,\{g\}$};
\node[learn, below=of cond] (learn) {\textbf{7. LEARN} \; $L.\text{apply\_match}(g,\lambda_{\text{obs}})$\\ regularised-Elo drift (zero-sum, shrunk to $R_0$)};
\node[sim, below left=7mm and 1mm of learn.south] (simf) {\textbf{8a.} simulate\\ \textbf{FROZEN} ($R_0$)\\ $\rightarrow$ champ.\ dist};
\node[sim, below right=7mm and 1mm of learn.south] (siml) {\textbf{8b.} simulate\\ \textbf{LEARNING} ($L$)\\ $\rightarrow$ champ.\ dist};
\node[pbox, below=7mm of learn, yshift=-15mm] (snap) {\textbf{9.} record \emph{after} snapshot (both tracks) $+$ KL vs.\ previous};
\node[dec, below=4mm of snap] (more) {more\\ games?};
\node[term, below=5mm of more] (out) {\textbf{OUTPUTS}\\
  $\bullet$ \textbf{trajectory} of before/after snapshots (frozen \& learning)\\
  $\bullet$ \textbf{KL} information per game (what mattered)\\
  $\bullet$ \textbf{A-vs-B} gap: did learning beat conditioning?};
\draw[a] (inp)--(init);
\draw[a] (init)--(t0);
\draw[a] (t0)--(loophead);
\draw[a] (loophead)--(bef);
...
\draw[a] (more.east) -- ++(50mm,0) node[midway,above,font=\scriptsize]{yes, next $g$} |- (loophead.east);
\end{tikzpicture}
