"""Mexico title-run scenario (Uzbekistan house style): ONE simulated
tournament in which the host wins it all (generator seed 623: R16 Brazil,
QF Czechia, SF Spain, FINAL the United States — a host-vs-host final),
champion probabilities traced match by match via condition.conditional_probs.
Writes archive/Mexico_World_Cup_scenario.{png,json}."""
import json
import sys
...
OUT = ROOT / "archive"
CACHE = OUT / "Mexico_title_trajectory.json"
GEN_SEED = 623
N = 8000

g_res, ko_res, champ = generate_realization(GEN_SEED)
assert champ == "Mexico", champ
...
if CACHE.exists():
    ...
    for i, (kind, m) in enumerate(steps, 1):
        ...
        if i % 12 == 0 or bits > 0.05:
            mex = champ_now.get("Mexico", 0.0)
            top = max(champ_now, key=champ_now.get)
            print(f"  [{i:3}/104] {label:<16} info={bits:5.3f} | "
                  f"Mexico {mex*100:4.1f}% | leader {top} {champ_now[top]*100:.0f}%",
                  flush=True)
...
TEAMS = [
    ("Mexico", TEAL, 4.5),
    ("Spain", "#cc2336", 2.2),
    ("United States", "#1f2440", 2.2),
    ("Brazil", "#e8a000", 2.2),
    ("Argentina", "#9a9a9a", 2.2),
]
...
for team, col, lw in TEAMS:
    ...
    lab = "USA (the other host)" if team == "United States" else team
    ax1.plot(ns, ys, color=col, lw=lw, label=lab,
             zorder=5 if team == "Mexico" else 3,
...
one_in = round(1 / champ0["Mexico"])
ax1.set_title(f"The 1-in-{one_in} world: MEXICO win on home soil "
...
ax1.annotate("...then beats BRAZIL (R16),\nSPAIN (SF) and the USA in a\n"
             "host-vs-host final = CHAMPION",
             xy=(103.6, 99), xytext=(65, 72),
...
ax2.annotate(f"{top['info_bits']:.2f} bits — the SEMI-FINAL upset of\n"
             "Spain is the shock, not the final\n"
             "(Mexico enter the final as 68% favorites)",
             xy=(top["n"], top["info_bits"] * 0.98),
             xytext=(46, top["info_bits"] * 0.55),
...
fig.text(0.045, 0.012,
         "One simulated path of the locked pre-registered model (Mexico carry the "
         "host bonus: +40 Elo, +20 from the quarter-finals).  Avraa's prediction model",
         fontsize=10, color="0.45")
fig.tight_layout(rect=(0, 0.025, 1, 1))
fig.savefig(OUT / "Mexico_World_Cup_scenario.png", dpi=150)
print("wrote archive/Mexico_World_Cup_scenario.png")
