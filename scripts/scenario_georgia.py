"""COUNTERFACTUAL Georgia title-run scenario (Uzbekistan house style).
Georgia did NOT qualify for 2026; for fun they replace Bosnia and Herzegovina
in Group B (the UEFA playoff-tier slot) at their real-world Elo 1660 (Bosnia:
...
locked model. Generator seed 10647: R16 Morocco, QF France, SF Spain, FINAL
Argentina. Baseline title chance from a 30,000-world scan: 4 wins = 1-in-7,500.
Writes archive/Georgia_World_Cup_scenario.{png,json}."""
import json
import math
import sys
...
OUT = ROOT / "archive"
CACHE = OUT / "Georgia_title_trajectory.json"
GEN_SEED = 10647
N = 8000
ONE_IN = 7500            # 4 titles in the 30,000-world scan

# ---- the counterfactual patch: Georgia in for Bosnia, Elo 1660 -------------
C.ELO["Georgia"] = 1660
...
for row in (10, 12, 15):
    ...
    h2 = "Georgia" if h == "Bosnia and Herzegovina" else h
    a2 = "Georgia" if a == "Bosnia and Herzegovina" else a
...
if CACHE.exists():
    ...
    for i, (kind, m) in enumerate(steps, 1):
        ...
        if i % 12 == 0 or bits > 0.05:
            geo = champ_now.get("Georgia", 0.0)
            top = max(champ_now, key=champ_now.get)
            print(f"  [{i:3}/104] {label:<16} info={bits:5.3f} | "
                  f"Georgia {geo*100:4.1f}% | leader {top} {champ_now[top]*100:.0f}%",
                  flush=True)
...
for team, col, lw in TEAMS:
    ...
    ax1.plot(ns, ys, color=col, lw=lw, label=team,
             zorder=5 if team == "Georgia" else 3,
...
ax1.set_title(f"The 1-in-{ONE_IN:,} world: GEORGIA win "
...
ax1.annotate("...then beats FRANCE (QF),\nSPAIN (SF) and ARGENTINA\n"
...
fig.text(0.045, 0.012,
         "Counterfactual for fun — Georgia did NOT qualify. They take Bosnia and "
         "Herzegovina's Group B slot at real-world Elo 1660; everything else is the "
         "locked model. 4 titles in 30,000 simulated worlds.  Avraa's prediction model",
         fontsize=10, color="0.45")
fig.tight_layout(rect=(0, 0.025, 1, 1))
fig.savefig(OUT / "Georgia_World_Cup_scenario.png", dpi=150)
print("wrote archive/Georgia_World_Cup_scenario.png")
